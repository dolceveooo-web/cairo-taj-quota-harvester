// ═══════════════════════════════════════════════════════════════════
// Cairo Taj — Dokki Line 094 — API-BASED HARVESTER
// Uses WE Egypt's REST API directly — no browser, no Puppeteer,
// no captcha, no login blocks. Pure HTTP calls.
// No line switching needed — account ID targets 094 directly.
// ═══════════════════════════════════════════════════════════════════
...
const WE_USERNAME         = process.env.DOKKI_USERNAME;  // e.g. 0237600094
...
async function weInit() {
  const res = await fetch(
    WE_BASE + '/echannel/service/besapp/base/rest/busiservice/v1/common/querySysParams',
    { method: 'POST', headers: WE_HEADERS, body: JSON.stringify({}) }
  );
  if (!res.ok) throw new Error('WE init failed: HTTP ' + res.status);
  console.log('  ✓ WE API session initialized');
}

async function weAuthenticate(acctId, password) {
  const res = await fetch(
    WE_BASE + '/echannel/service/besapp/base/rest/busiservice/v1/auth/userAuthenticate',
    {
      method: 'POST',
      headers: WE_HEADERS,
      body: JSON.stringify({ acctId, appLocale: 'en-US', password })
    }
  );
  if (!res.ok) throw new Error('WE auth HTTP ' + res.status);
  const json = await res.json();
  if (json.header.retCode !== '0') throw new Error('WE auth failed: ' + (json.header.retMsg || json.header.retCode));
  const { customer, subscriber, token } = json.body;
...
async function weGetOfferId(acctId, token) {
  const res = await fetch(
    WE_BASE + '/echannel/service/besapp/base/rest/busiservice/cz/v1/auth/getSubscribedOfferings',
    {
      method: 'POST',
      headers: { ...WE_HEADERS, csrftoken: token },
      body: JSON.stringify({ msisdn: acctId, numberServiceType: 'FBB', groupId: '' })
    }
  );
  if (!res.ok) throw new Error('WE offerings HTTP ' + res.status);
  const json = await res.json();
  if (json.header.retCode !== '0') throw new Error('WE offerings failed: ' + json.header.retCode);
  const offerId = json.body.offeringList[0].mainOfferingId;
...
async function weGetQuota(token, subscriberId, offerId) {
  const res = await fetch(
    WE_BASE + '/echannel/service/besapp/base/rest/busiservice/cz/cbs/bb/queryFreeUnit',
    {
      method: 'POST',
      headers: { ...WE_HEADERS, csrftoken: token },
      body: JSON.stringify({ subscriberId, mainOfferId: offerId })
    }
  );
  if (!res.ok) throw new Error('WE quota HTTP ' + res.status);
  const json = await res.json();
  if (json.header.retCode !== '0') throw new Error('WE quota failed: ' + json.header.retCode);
  const q = json.body[0];
  return {
    remaining: q.remain,
    used:      q.used,
...
async function firestoreWrite(data, updatedBy) {
  ...
  console.log('  ✓ Firestore updated (quota_latest/current → dokki)');
}
...
async function firestoreFlag(remaining) {
  ...
  console.log('  ✓ Alert flag: dokki_low=' + isLow + ' (' + remaining.toFixed(1) + ' GB)');
}
...
async function telegramSend(text) {
  ...
  for (const chatId of recipients) {
    try {
      ...
      if (!res.ok) console.log('  ⚠ Telegram to ' + chatId + ': HTTP ' + res.status);
...
async function harvestQuota() {
  console.log('🚀 STARTING API HARVEST — Dokki Line 094\n');

  // Account ID: FBB + number without leading 0
  ...
  const msg = [
    ...
    '📋 Plan: ' + data.plan,
    '🕐 ' + date,
    '🤖 API Harvester ⚡ Dokki' + extra
  ].join('\n');
  await telegramSend(msg);
  if (rem < 30) {
    await telegramSend(['🚨🚨🚨 *CRITICAL QUOTA ALERT* 🚨🚨🚨', '', '⚠️ *Cairo Taj — Dokki*', '📉 Only *' + rem.toFixed(2) + ' GB* remaining!', '🔴 *ACTION REQUIRED: Recharge immediately!*', '', '🕐 ' + date].join('\n'));
  }
  ...
  // ══════════════════════════════════════════════════════════════
  // VIGILANCE MODE — quota ≤ 50 GB (Dokki)
  // Re-calls quota API every 13 min — same token, zero new logins
  // ══════════════════════════════════════════════════════════════
  if (data.remaining <= 50) {
    console.log('\n🔴 VIGILANCE MODE ACTIVATED (DOKKI) — ' + data.remaining.toFixed(2) + ' GB ≤ 50 GB');
    console.log('  Every 13 min via API. Stops at ≤ 2 GB or 5h45m.\n');
    ...
    while (true) {
      ...
      console.log('\n' + '═'.repeat(50));
      console.log('⚡ VIGILANCE ROUND #' + round + ' — DOKKI (' + elapsedMin + 'min elapsed)');
      ...
      try {
        ...
        if (vRem <= 10) {
          await telegramSend(['🚨🚨🚨 *VIGILANCE CRITICAL* 🚨🚨🚨', '', '⚠️ *Cairo Taj — Dokki*', '📉 Only *' + vRem.toFixed(2) + ' GB* remaining!', '🔴 *Recharge immediately!*', '', '🕐 ' + vDate].join('\n'));
        }
    ...
    console.log('[VIGILANCE] Exiting after ' + round + ' rounds (Dokki).');
  }
}
