...
async function harvestQuota() {
  ...
  async function loadSavedCookies() {
    try {
      ...
      if (age > 8 * 60 * 60 * 1000) { console.log('  [SESSION] Cookies expired (>8h old), fresh login'); return null; }
  ...
  try {
    ...
    try {
      ...
      if (relevantCookies.length > 0) await saveCookies(relevantCookies, storageData);
    } catch(e) { console.log('  [SESSION] Could not save cookies:', e.message); }
