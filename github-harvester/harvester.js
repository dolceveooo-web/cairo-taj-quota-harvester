...
async function harvestQuota() {
  ...
  try {
    ...
    await tryMethods([
      async () => {
        const mask = 'updateMask.fieldPaths=104&updateMask.fieldPaths=lastUpdate';
        ...
        console.log('    updateMask PATCH (104 field)');
      },
      async () => {
        await sleep(2000);
    ...
    // ══════════════════════════════════════════════════════════════
    // VIGILANCE MODE — triggered when quota ≤ 50 GB (Line 104)
    // ══════════════════════════════════════════════════════════════
    if (data.remaining <= 50) {
      console.log('\n🔴 VIGILANCE MODE ACTIVATED (LINE 104) — quota=' + data.remaining.toFixed(2) + ' GB ≤ 50 GB');
      ...
      while (true) {
        ...
        console.log('\n' + '═'.repeat(50));
        console.log('⚡ VIGILANCE ROUND #' + vigilanceRound + ' — LINE 104 (' + elapsedMin + 'min elapsed)');
