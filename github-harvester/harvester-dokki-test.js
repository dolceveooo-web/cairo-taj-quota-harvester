...
async function harvestQuota() {
  ...
  // ظ¤ظ¤ Session Management: Save/Load ALL session data (Dokki) ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤ظ¤
  ...
  try {
    ...
    // extract the data THE MOMENT we confirm the correct page is showing ظ¤
    ...
    await tryMethods([
      // M1: Click dropdown ظْ select 0237600094 ظْ capture data immediately on confirmation
      async () => {
          ...
          if (check.hasRemaining && check.has094) {
            ...
            if (captured) {
              ...
              if (captured.balance > 3000 && captured.balance > 0 && captured.plan !== 'Unknown' && totalGB > 300) {
                ...
                console.log('    ظ£ô M1 FULL DATA CAPTURED: remaining=' + captured.remaining + ' used=' + captured.used + ' total=' + totalGB.toFixed(1) + 'GB balance=' + captured.balance + ' plan=' + captured.plan);
                return; // SUCCESS
              } else if (captured.balance > 0 && captured.balance < 3000) {
                console.log('    ظأب (' + (w+1) + 's) Balance ' + captured.balance + ' < 3000 ظ¤ WRONG LINE (093), waiting for 094...');
              } else if (captured.balance === 0) {
                console.log('    ظ│ (' + (w+1) + 's) Balance=0, page still loading... rem=' + captured.remaining);
              } else if (captured.plan === 'Unknown') {
                console.log('    ظ│ (' + (w+1) + 's) Plan=Unknown, page still rendering... rem=' + captured.remaining + ' bal=' + captured.balance);
              } else if (totalGB <= 300) {
                console.log('    ظأب (' + (w+1) + 's) MIXED STATE: balance=' + captured.balance + ' (094ظ£ô) but rem+used=' + totalGB.toFixed(1) + 'GB (093 plan=250GB!) ظ¤ waiting for full 094 data...');
              } else {
                console.log('    ظ│ (' + (w+1) + 's) Data incomplete, waiting... rem=' + captured.remaining + ' bal=' + captured.balance + ' total=' + totalGB.toFixed(1));
              }
            } else {
              console.log('    ظ│ (' + (w+1) + 's) extractNow returned null, waiting...');
            }
          } else {
            console.log('    ظ│ (' + (w+1) + 's) URL:' + url.split('#')[1] + ' | has094:' + check.has094 + ' | hasRem:' + check.hasRemaining + ' | bal:' + check.balNum);
          }
        }
        throw new Error('M1: Page did not show line 94 FULL data (balance>3000, totalGB>300, plan loaded) in 30s');
      },

      // M2: Broad evaluate click ظْ same capture strategy
      async () => {
        await sleep(2000);
        ...
        for (let w = 0; w < 25; w++) {
          ...
          if (check.hasRemaining && check.has094) {
            ...
            if (captured) {
              ...
              if (captured.balance > 3000 && captured.balance > 0 && captured.plan !== 'Unknown' && totalGB > 300) {
                switcherCapturedData = captured;
                console.log('    ظ£ô M2 FULL DATA CAPTURED: remaining=' + captured.remaining + ' total=' + totalGB.toFixed(1) + 'GB balance=' + captured.balance);
                return;
              } else if (captured.balance > 0 && captured.balance < 3000) {
                console.log('    ظأب (' + (w+1) + 's) Balance ' + captured.balance + ' < 3000 ظ¤ WRONG LINE (093)');
              } else if (captured.balance === 0) {
                console.log('    ظ│ (' + (w+1) + 's) Balance=0, loading... rem=' + captured.remaining);
              } else if (captured.plan === 'Unknown') {
                console.log('    ظ│ (' + (w+1) + 's) Plan=Unknown, rendering... rem=' + captured.remaining + ' bal=' + captured.balance);
              } else if (totalGB <= 300) {
                console.log('    ظأب (' + (w+1) + 's) MIXED STATE: balance=' + captured.balance + 'ظ£ô but total=' + totalGB.toFixed(1) + 'GB = 093 plan, waiting...');
              }
            }
          } else {
            console.log('    ظ│ (' + (w+1) + 's) rem:' + check.rem + ' | has094:' + check.has094 + ' | bal:' + check.balNum);
          }
        }
        ...
        for (let w = 0; w < 25; w++) {
          ...
          if (check.hasRemaining && check.has094) {
            ...
            if (captured) {
              ...
              if (captured.balance > 3000 && captured.balance > 0 && captured.plan !== 'Unknown' && totalGB > 300) {
                switcherCapturedData = captured;
                console.log('    ظ£ô M3 FULL DATA CAPTURED: remaining=' + captured.remaining + ' total=' + totalGB.toFixed(1) + 'GB balance=' + captured.balance);
                return;
              } else if (captured.balance > 0 && captured.balance < 3000) {
                console.log('    ظأب (' + (w+1) + 's) Balance ' + captured.balance + ' < 3000 ظ¤ WRONG LINE (093)');
              } else if (captured.balance === 0) {
    ...
    console.log('  ظ£ô Switched to 0237600094 | captured data:', switcherCapturedData ? 'YES' : 'NO');
    ...
    const data = switcherCapturedData ? await (async () => {
      ...
      // M1: Walk ALL spans/divs ظ¤ numeric sibling scan
      async () => {
        await sleep(2000);
        const result = await page.evaluate(() => {
          const spans = Array.from(document.querySelectorAll('span, div, p'));
    ...
    const fields = {
      'dokki': { mapValue: { fields: {
        ...
        updatedBy: { stringValue: 'GitHub Cloud ظأة Dokki' },
    ...
    const historyFields = {
      ...
      user: { stringValue: 'GitHub Cloud ظأة Dokki' },
      notes: { stringValue: '' },
      dokki: { mapValue: { fields: {
        quota: { doubleValue: data.remaining },
        balance: { doubleValue: data.balance }
    ...
    // dokki_low: true  ظْ hourly workflow will run full harvest
    // dokki_low: false ظْ hourly workflow will skip (normal 2h schedule handles it)
    try {
      ...
      if (alertRes.ok) {
        console.log('  ظ£ô Low quota flag set: dokki_low=' + isLowDokki + ' (' + data.remaining.toFixed(1) + ' GB)\n');
      } else {
    ...
    try {
      ...
      const msg = [
        ...
        `≡اôï Plan: ${data.plan}`,
        `≡اـ ${date}`,
        `≡اجû GitHub Cloud ظأة Dokki` + alertLine
      ].join('\n');
      ...
      if (rem < 30) {
        const criticalMsg = {
          text: ['≡اأذ≡اأذ≡اأذ *CRITICAL QUOTA ALERT* ≡اأذ≡اأذ≡اأذ', '', 'ظأبي╕ *Cairo Taj ظ¤ Dokki*',
    ...
    try {
      ...
      if (error.message && (error.message.includes('Still on login page') || error.message.includes('navigation or captcha'))) {
        ...
        if (attempt === MAX_RETRIES) {
          try {
            const msg = `ظأبي╕ Dokki Login Failed (All ${MAX_RETRIES} Attempts)\n\nIssue: ${error.message}\n\nCheck GitHub Actions logs for details.\n\nTime: ${new Date().toLocaleString('en-US', {timeZone: 'Africa/Cairo'})} Cairo`;
            await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ chat_id: TELEGRAM_CHAT_ID, text: msg })
